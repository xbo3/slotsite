    ...
    <div className="flex justify-center px-3 py-6 min-h-screen" style={{ fontFamily: "'Poppins', sans-serif", WebkitFontSmoothing: 'antialiased' }}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-normal tracking-[2px] uppercase text-white">{t('deposit_title')}</h2>
          <button
            onClick={() => router.back()}
        ...
        <div className="flex px-5 pt-3 gap-[3px]">
          {(['bank', 'coin'] as Tab[]).map((t2) => (
            <button
              key={t2}
              onClick={() => setTab(t2)}
              className="flex-1 py-[11px] text-xs font-light tracking-[1px] relative overflow-hidden transition-all duration-[250ms]"
              style={{
                background: tab === t2 ? '#181818' : 'rgba(255,255,255,0.04)',
                color: tab === t2 ? '#fff' : '#555',
                borderRadius: '10px 10px 0 0',
                border: tab === t2 ? '1px solid rgba(255,255,255,0.1)' : '1px solid transparent',
                borderBottom: tab === t2 ? 'none' : '1px solid transparent',
              }}
            >
              {tab === t2 && <span className="absolute inset-0" style={shineStyle} />}
